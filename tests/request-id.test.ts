...
import { requestId as createRequestIdMiddleware } from "../src/request-id.ts";
...
describe("Request ID 中间件", () => {
  describe("基础功能", () => {
    ...
    it("应该将 Request ID 存储到 context 中", async () => {
      ...
      const id = (context as any).state?.requestId;
      expect(id).toBeDefined();
      expect(typeof id).toBe("string");
    });
    ...
    it("应该使用请求头中的 Request ID（如果存在）", async () => {
      ...
      expect((context as any).state?.requestId).toBe(existingId);
    });
  ...
  describe("配置选项", () => {
    ...
    it("应该支持禁用响应头中的 Request ID", async () => {
      ...
      const id = context.response?.headers.get("X-Request-ID");
      expect(id).toBeNull();
      // 但 context.state 中仍然应该有 Request ID
      expect((context as any).state?.requestId).toBeDefined();
    });
